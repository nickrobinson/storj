...
type Client interface {
	...
	PayerBandwidthAllocation(context.Context, pb.BandwidthAction, int64) (*pb.PayerBandwidthAllocation, error)
...
func (pdb *PointerDB) PayerBandwidthAllocation(ctx context.Context, action pb.BandwidthAction, maxSize int64) (resp *pb.PayerBandwidthAllocation, err error) {
	defer mon.Task()(&ctx)(&err)

	response, err := pdb.client.PayerBandwidthAllocation(ctx, &pb.PayerBandwidthAllocationRequest{Action: action, MaxSize: maxSize})
	if err != nil {
		return nil, err
	}
